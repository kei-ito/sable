...
const {contentTypes, Injector} = require('../util.js');
const readdir = promisify(fs.readdir);
const stat = promisify(fs.stat);
const scriptPath = path.join(__dirname, 'autoreload.script.js');
exports.staticFile = ({
	...
	const respondIndex = async (res, url, directory) => {
		...
		writer.write('<script>\n');
		writer.write('window.addEventListener(\'error\', function (e) {document.body.insertAdjacentHTML(\'afterbegin\', String(e.stack || e))})\n');
		writer.write('</script>\n');
		writer.end();
	};
