...
var toASCII$1 = function toASCII$1(domain_name, useSTD3, processing_option, verifyDnsLength) {
...
var toUnicode$1 = function toUnicode$1(domain_name, useSTD3) {
...
var urlStateMachine = createCommonjsModule(function (module) {
  ...
  function parseIPv6(input) {
    var ip = [0, 0, 0, 0, 0, 0, 0, 0];
    var piecePtr = 0;
    var compressPtr = null;
    var pointer = 0;

    input = punycode$2.ucs2.decode(input);
    ...
    if (compressPtr !== null) {
      var swaps = piecePtr - compressPtr;
      piecePtr = 7;
      while (piecePtr !== 0 && swaps > 0) {
        var temp = ip[compressPtr + swaps - 1]; // piece
  ...
  function parseHost(input, isSpecialArg) {
    ...
    var asciiDomain = index$2.toASCII(domain, false, index$2.PROCESSING_OPTIONS.NONTRANSITIONAL, false);
    if (asciiDomain === null) {
      return failure;
    }
  ...
  function parseOpaqueHost(input) {
    ...
    var output = "";
    var decoded = punycode$2.ucs2.decode(input);
  ...
  function URLStateMachine(input, base, encodingOverride, url, stateOverride) {
    ...
    this.input = punycode$2.ucs2.decode(this.input);
  ...
  URLStateMachine.prototype["parse file"] = function parseFile(c) {
    ...
    if (c === 47 || c === 92) {
      ...
      if (isNaN(c)) {
        ...
        if (!isWindowsDriveLetterCodePoints(c, this.input[this.pointer + 1]) || this.input.length - this.pointer - 1 === 1 || // remaining consists of 1 code point
  ...
  function serializeOrigin(tuple) {
    ...
    result += index$2.toUnicode(tuple.host, false).domain;
  ...
  module.exports.setTheUsername = function (url, username) {
    url.username = "";
    var decoded = punycode$2.ucs2.decode(username);
    for (var i = 0; i < decoded.length; ++i) {
  ...
  module.exports.setThePassword = function (url, password) {
    url.password = "";
    var decoded = punycode$2.ucs2.decode(password);
    for (var i = 0; i < decoded.length; ++i) {
...
var implementation = function () {
  function URLImpl(constructorArgs) {
    ...
    var parsedURL = urlStateMachine.basicURLParse(url, { baseURL: parsedBase });
  ...
  _createClass(URLImpl, [{
    ...
    key: 'origin',
    get: function get() {
      return urlStateMachine.serializeURLToUnicodeOrigin(this._url);
    }
  }, {
    key: 'protocol',
    get: function get() {
    ...
    set: function set(v) {
      ...
      url.query = "";
      urlStateMachine.basicURLParse(input, { url: url, stateOverride: "query" });
    }
  }, {
    key: 'hash',
    get: function get() {
...
var URL_1 = createCommonjsModule(function (module) {
  ...
  function URL(url) {
    ...
    args[0] = index["USVString"](args[0]);
    if (args[1] !== undefined) {
      args[1] = index["USVString"](args[1]);
    }
...
function replaceCSS(file) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    var url = new URL(getAttribute(linkElement, 'href'), currentURL);
    url.search = '';
    url.hash = '';
