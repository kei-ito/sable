(function () {
	...
	getBody(function () {
		...
		const connect = function (error) {
			...
			connect.timer = setTimeout(function () {
		...
		const findCSS = function (file) {
			...
			if (0 < toBeReplaced.length) {
				reloadCSS(
					toBeReplaced,
					[updatedFileFragments.join('/'), '?d=', Date.now()].join('')
				);
			}
		};

		const reloadCSS = function (currentLinkElements, href) {
