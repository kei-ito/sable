...
test('SableServer', (test) => {
	...
	const testRootDirectory = path.join(__dirname, 'temp', `${Date.now()}`);
	...
	test('documentRoot', (test) => {
		...
		const testDirectory = path.join(testRootDirectory, 'documentRoot');
	...
	test('start/close', (test) => {
		...
		const testDirectory = path.join(testRootDirectory, 'start-close');
